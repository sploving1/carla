...
class BasicSynchronousClient(object):
    ...
    '''def render(self, display):
        ...
        if self.image is not None:
            ...
            # draw labels
            #image = cv2.resize(array, None, fx=1, fy=1)
            ...
            array = draw_labels(boxes, confs, colors, class_ids, classNames, image)
    ...
    def render(self, display):
        if self.image is not None:
            ...
            i3 = i2[:, :, :3]
            #self.depth = i3
            image = cv2.cvtColor(i3,cv2.COLOR_RGB2BGR)
            blob = cv2.dnn.blobFromImage(image, 1 / 255, (VIEW_WIDTH, VIEW_HEIGHT), swapRB=True, crop=False)
            net.setInput(blob)
            outputs = net.forward(outputlayers)
            ...
            cv2.imshow("前面摄像头", cv2.cvtColor(image,cv2.COLOR_RGB2BGR))
    ...
    def depth_render(self, depth_display):
        if self.depth_image is not None:
            ...
            cv2.imshow("后面摄像头", cv2.cvtColor(image,cv2.COLOR_RGB2BGR))
    ...
    def game_loop(self):
        ...
        try:
            ...
            self.display = cv2.namedWindow('前面摄像头')
            self.depth_display = cv2.namedWindow('后面摄像头')
            ...
            tm = self.client.get_trafficmanager(3007)
            tm_port = tm.get_port()
